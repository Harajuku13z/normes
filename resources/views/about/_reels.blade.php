@php
    ...
    $validReels = collect($reels ?? [])
        ->filter(fn ($r) => is_array($r) && trim((string) data_get($r, 'video', '')) !== '')
        ->values();
    $reelCount = $validReels->count();
@endphp

@if ($reelCount > 0)
<section aria-label="Vidéos Normes et Rénovation">

    {{-- ══ MOBILE : plein écran TikTok-style ══ --}}
    <div class="relative h-[100svh] lg:hidden">
        <div class="reels-scroll h-full snap-y snap-mandatory overflow-y-auto" id="reels-mobile">
            @foreach ($validReels as $idx => $reel)
                @php
                    $videoUrl = HomeView::url((string) data_get($reel, 'video'));
                ...
                <div class="reel-slide relative h-[100svh] w-full snap-start" data-index="{{ $idx }}">
                    ...
                    ></video>
                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/25" aria-hidden="true"></div>

                    @if ($caption !== '')
                        <div class="absolute bottom-16 left-5 right-14 z-10">
        ...
        {{-- Indicateur de points (fixe) --}}
        <div class="pointer-events-none absolute right-4 top-1/2 z-20 -translate-y-1/2 flex flex-col gap-2" id="reels-dots">
        ...
        <button
            type="button"
            class="absolute right-4 top-6 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-black/40 text-white backdrop-blur-sm transition hover:bg-black/60"
            id="reels-mute-btn"
            aria-label="Activer le son"
        >
            <svg id="icon-muted" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"/><path stroke-linecap="round" stroke-linejoin="round" d="M17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2"/></svg>
            <svg id="icon-unmuted" class="hidden h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.536 8.464a5 5 0 010 7.072M17.95 6.05a8 8 0 010 11.9M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"/></svg>
        </button>

        {{-- Indicateur swipe (premier reel) --}}
        <div class="pointer-events-none absolute bottom-4 left-1/2 z-20 -translate-x-1/2 animate-bounce" id="reels-swipe-hint">
    ...
    <div class="hidden bg-brand-dark py-20 sm:py-24 lg:block">
        <div class="mx-auto w-[95%] px-4 sm:px-6 lg:px-8">
            ...
            <div class="mx-auto mt-14 grid max-w-5xl grid-cols-4 gap-5">
