    ...
    <div class="mx-auto grid w-[95%] gap-4 px-4 sm:grid-cols-2 sm:gap-5 sm:px-6 lg:grid-cols-4 lg:gap-6 lg:px-8">
        @foreach (data_get($h, 'stats.items', []) as $item)
            <article class="flex flex-col items-center rounded-2xl border border-white/15 bg-white px-4 py-7 text-center shadow-soft ring-1 ring-white/10 sm:py-8">
                <span class="mb-4 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-blue/10 text-brand-blue ring-1 ring-brand-blue/15" aria-hidden="true">
                ...
                <strong class="text-4xl font-black tracking-tight text-brand-blue">{{ data_get($item, 'value') }}</strong>
                <span class="mt-1 text-sm font-bold text-brand-blue/90 sm:text-base">{{ data_get($item, 'label') }}</span>
            </article>
        @endforeach
    </div>
</section>
