...
@section('content')
        ...
        <details class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" open>
            ...
            <div class="mt-4">
                ...
                <div class="mb-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs font-extrabold uppercase tracking-wide text-slate-500">SEO page contact</p>
                    <div class="mt-3 grid gap-4 lg:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Meta title</label>
                            <input type="text" name="sections[contact_page][meta_title]" value="{{ data_get($cp, 'meta_title') }}" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="mb-1 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Meta keywords</label>
                            <input type="text" name="sections[contact_page][meta_keywords]" value="{{ data_get($cp, 'meta_keywords') }}" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        </div>
                        <div class="lg:col-span-2">
                            <label class="mb-1 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Meta description</label>
                            <textarea name="sections[contact_page][meta_description]" rows="2" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">{{ data_get($cp, 'meta_description') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <p class="text-xs font-extrabold uppercase tracking-wide text-slate-500">Image mise en avant (Open Graph)</p>
                        <input id="cpFeaturedImage" type="text" name="sections[contact_page][featured_image]" value="{{ data_get($cp, 'featured_image') }}" class="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        <input id="cpFeaturedImageUpload" type="file" accept="image/*" class="mt-2 w-full text-sm">
                        <label class="mb-1 mt-3 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Image OG (si différente)</label>
                        <input id="cpOgImage" type="text" name="sections[contact_page][og_image]" value="{{ data_get($cp, 'og_image', data_get($cp, 'featured_image')) }}" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                    </div>
                </div>
                ...
                <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <p class="text-xs font-extrabold uppercase tracking-wide text-slate-500">Libellés courts (titres en capitales)</p>
                    <div class="mt-3 grid gap-4 lg:grid-cols-3">
                        <div>
                            <label class="mb-1 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Libellé “Carte”</label>
                            <input type="text" name="sections[contact_page][labels][map]" value="{{ data_get($cp, 'labels.map') }}" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="mb-1 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Libellé “Réseaux sociaux”</label>
                            <input type="text" name="sections[contact_page][labels][social]" value="{{ data_get($cp, 'labels.social') }}" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="mb-1 block text-xs font-extrabold uppercase tracking-wide text-slate-500">Libellé “Horaires”</label>
                            <input type="text" name="sections[contact_page][labels][hours]" value="{{ data_get($cp, 'labels.hours') }}" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        uploadToInput('cpFeaturedImageUpload', 'cpFeaturedImage');
        document.getElementById('cpFeaturedImage')?.addEventListener('input', (e) => {
            const og = document.getElementById('cpOgImage');
            if (og && !og.value) og.value = e.target.value || '';
        });
